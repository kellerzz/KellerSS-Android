...
$vermelho . "\n[!] Opção inválida! Tente novamente. \n\n" . $cln;
    ...
    else
    {
        if ($opcaoscanner == "0") {
            system("clear");
            keller_banner();
            echo $bold . $fverde . "[+] Instalando Android Tools (necessário para o ADB)...\n" . $cln;
            system("pkg install -y android-tools > /dev/null 2>&1");
            echo $bold . $fverde . "[i] Instalação concluída!\n\n" . $cln;
            ...
            $comandoDispositivos = shell_exec("adb devices 2>&1");
                ...
                if (!empty($comandoVerificarFF) && strpos($comandoVerificarFF, "more than one device/emulator") !== false) {
                    echo $bold .
$vermelho . "[!] Pareamento realizado de maneira incorreta, digite \"adb disconnect\" e refaça o processo.\n\n";
                    exit;
                }
                ...
                if (!empty($comandoVerificarFF) && strpos($comandoVerificarFF, "com.dts.freefireth") !== false) {
                    ...
                    echo $bold .
$vermelho . "[!] O FreeFire está desinstalado, cancelando a telagem...\n\n";
                    exit;
                }
                ...
                if (!empty($resultadoVersaoAndroid)) {
                    echo $bold .
$azul . "[+] Versão do Android: " . trim($resultadoVersaoAndroid) . "\n";
                } else {
                    echo $bold .
$vermelho . "[!] Não foi possível obter a versão do Android.\n";
                }
                ...
                if (!empty($resultadoSu) && strpos($resultadoSu, 'No su program found') !== false) {
                    echo $bold .
$fverde . "[-] O dispositivo não tem root.\n\n";
                } else {
                    echo $bold .
$vermelho . "[+] Root detectado no dispositivo Android.\n\n";
                }
                


            echo $bold .
$azul . "[+] Checando se o dispositivo foi reiniciado recentemente...\n";
            ...
            if (preg_match('/up (\d+) min/', $comandoUPTIME, $filtros)) {
                ...
                echo $bold .
$fverde . "[i] Dispositivo não reiniciado recentemente.\n\n";
            }
            ...
            preg_match('/(\d{2}-\d{2} \d{2}:\d{2}:\d{2})/', $logcatTime, $matchTime);
                ...
                if (!empty($matchTime[1])) {
                    ...
                    echo $bold .
$vermelho . "[!] Não foi possível capturar a data/hora do sistema.\n\n";
                }
            
            echo $bold .
$azul . "[+] Verificando mudanças de data/hora...\n";
            ...
            if ($logcatOutput !== null && trim($logcatOutput) !== "") {
                ...
                echo $bold .
$vermelho . "[!] Erro ao obter logs de modificação de data/hora, verifique a data da primeira log do sistema.\n\n";
            }
            ...
            if ($fusoHorario !== "America/Sao_Paulo") {
                echo $bold .
$amarelo . "[!] Aviso: O fuso horário do dispositivo é '$fusoHorario', diferente de 'America/Sao_Paulo', possivel tentativa de Bypass.\n\n";
            }
...
'Atrasou' : 'Adiantou',
            ...
            if (!empty($logsAlterados)) {
                ...
                foreach ($logsAlterados as $log) {
                    echo $bold .
$amarelo . "[!] Alterou horário de {$log['dataAntiga']} para {$log['dataNova']} {$log['horaNovaFormatada']} ({$log['acao']} horário)\n";
                }
            } else {
                echo $bold .
$vermelho . "[!] Nenhum log de alteração de horário encontrado.\n\n";
            }
...
$azul . "\n[+] Checando se modificou data e hora...\n";
            ...
            if ($autoTime !== "1" || $autoTimeZone !== "1") {
                echo $bold .
$vermelho . "[!] Possível bypass detectado: data e hora/furo horário automático desativado.\n";
            } else {
                echo $bold .
$fverde . "[i] Data e hora/fuso horário automático estão ativados.\n";
            }

            echo $bold . $branco .
"[+] Caso haja mudança de horário durante/após a partida, aplique o W.O!\n\n";


            echo $bold . $azul .
"[+] Obtendo os últimos acessos do Google Play Store...\n";
            ...
            if (!is_null($comandoUSAGE) && trim($comandoUSAGE) !== "") {
                echo $bold .
$fverde . "[i] Últimos 5 acessos:\n";
                echo $amarelo . $comandoUSAGE . "\n";
            } else {
                echo $bold .
"\e[31m[!] Nenhum dado encontrado.\n";
            }
            echo $bold .
$branco . "[+] Caso haja acesso durante/após a partida, aplique o W.O!\n\n";

            echo $bold . $azul .
"[+] Obtendo os últimos textos copiados...\n";
            ...
            if (!is_null($saida)) {
                ...
                foreach ($linhas as $linha) {
                    if (!empty($linha) && preg_match('/^([0-9]{2}-[0-9]{2}) ([0-9]{2}:[0-9]{2}:[0-9]{2}) (.+)$/', $linha, $matches)) {
                        ...
                        echo $bold . $amarelo . "[!] " . $data . " " .
$hora . " " . $branco . "$conteudo" . "\n";
                    }
                }
            } else {
                echo $bold .
...
$dataAccess) ||
                            ...
                            preg_match('/\.0+$/', $dataChange)
                             ...
                             $motivos[] = "Motivo 2 - Timestamps com .000 " .
basename($arquivo);
                        }
                            ...
                            if (preg_match('/(\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2})/', basename($arquivo), $match)) {
                                ...
                                if ($nomeTimestamp !== false && $modifyTimestamp !== false) {
                                    ...
                                    if ($diffNs > 1_000_000_000) { 
                                        $motivos[] = "Motivo 4 - Nome do arquivo não bate com Modify: " .
basename($arquivo);
                                    }
                                } else {
...
basename($jsonPath);
                        ...
                        if (!$jsonStat) {
                            $motivos[] = "Motivo 8 - Arquivo JSON ausente: " .
basename($jsonPath);
                        }
                            ...
                            if (preg_match('/(\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2})/', basename($arquivoMaisRecente), $match)) {
                                ...
                                if (preg_match('/\.(\d{2})(\d+)/', $timestamps['Access'], $milisegundosMatch)) {
                                    ...
                                    if (($todosZeros || $condicaoValida) && $nomeNormalizado !== $modifyPastaNormalizado) {
                                        $motivos[] = "Motivo 9 - Nome não bate com Modify da pasta + milissegundos suspeitos" .
basename($arquivoMaisRecente);
                                    }
                                }
                            }
                        }
                ...
                if (!empty($motivos)) {
                    echo $bold .
$vermelho . "[!] Passador de replay detectado, aplique o W.O!\n";
...
$motivo . "\n";
                    ...
                    echo $bold .
$fverde . "[i] Nenhum replay foi passado e a pasta MReplays está normal.\n";
                }
                ...
                if (!empty($resultadoPasta)) {
                    ...
                    if (!empty($matchAccessPasta[1])) {
                        ...
                        echo $bold .
$amarelo . "[+] Data de acesso da pasta MReplays: $dataFormatada\n";
                        echo $bold . $amarelo .
"[*] Data de instalação do Free Fire: $dataInstalacaoFormatada\n";
                        echo $bold . $branco .
"[#] Verifique a data de instalação do jogo com a data de acesso da pasta MReplays para ver se o jogo foi recém instalado antes da partida, se não, vá no histórico e veja se o player jogou outras partidas recentemente, se sim, aplique o W.O!\n\n";
                    } else {
                        echo $bold .
$vermelho . "[!] Não foi possível obter a data de acesso da pasta MReplays\n\n";
                    }
                }
                ...
                echo $bold .
$azul . "[+] Checando bypass de Wallhack/Holograma...\n";
                ...
                foreach ($pastasParaVerificar as $pasta) {
                    $comandoStat = 'adb shell stat ' .
escapeshellarg($pasta) . ' 2>&1';
                    ...
                    if (strpos($resultadoStat, 'File:') !== false) {
                        ...
                        if ($matchModify && $matchChange) {
                            ...
                            if ($dataModifyFormatada !== $dataChangeFormatada) {
                                ...
                                echo $bold . $vermelho .
"[!] Bypass de renomear/substituir na pasta: $nomefinalpasta! Confira se o horário é após a partida, se sim, aplique o W.O!\n";
                ...
                $comandoFindBin = 'adb shell ls -t "/sdcard/Android/data/com.dts.freefireth/files/MReplays" |
grep "\.bin$" | head -n 1';
...
escapeshellarg($pasta) . ' 2>&1';
                        ...
                        if ($bypassDetectado) {
                               ...
                               echo $bold . $vermelho .
"[!] Modificando pastas após o fim da partida, aplique o W.O!\n\n";
                        } else {
                            echo $bold .
$verde . "[+] Nenhum bypass de holograma detectado.\n\n";
                        }
                    } else {
                        echo $bold .
$amarelo . "[!] Não foi possível obter a data do último .bin.\n";
                    }
                } else {
                    echo $bold .
$vermelho . "[!] Nenhum .bin encontrado em MReplays.\n";
                }
                        ...
                        if (strpos($resultadoStat, 'File:') !== false) {
                            ...
                            if ($matchAccess && $matchModify && $matchChange) {
                                ...
                                if ($accessDate === $modifyDate && $modifyDate === $changeDate) {
                                    ...
                                    $dataAccessFormatada = $dateTimeAccess ?
$dateTimeAccess->format('d-m-Y H:i:s') : $accessDate;
                                    ...
                                    echo $bold .
$laranja . "[!] Data da modificação (Access/Modify/Change): $dataAccessFormatada\n";
                                ...
                                if ($modifyDate !== $changeDate) {
                                    ...
                                    echo $bold . $vermelho .
"[!] Verifique se a data é após a partida, se sim aplique o W.O!\n\n";
                                }
                            }
                        }
                    }
                     ...
                     echo $bold . $amarelo .
"[i] Nenhum arquivo de shader encontrado.\n";
                }
                        ...
                        if (!empty($resultadoStat) && strpos($resultadoStat, "No such file or directory") === false) {
                            ...
                            if (!empty($matchModify[1]) && !empty($matchChange[1]) && !empty($matchAccess[1])) {
                                ...
                                if ($diffInSeconds <= 3600) {
                                    echo $bold .
$amarelo . "[!] Possível bypass detectado: arquivo shader alterado recentemente.\n";
                                    echo $bold . $amarelo . "[!] Arquivo: $nomeArquivo\n";
                                    echo $bold . $amarelo . "[*] Hora da modificação: $dataModify\n";
                                    echo $bold . $amarelo . "[*] Hora atual: " .
$currentDateTime->format('d-m-Y H:i:s') . "\n\n";
                                ...
                                if ($dataModify === $matchChange[1] && $dataModify === $matchAccess[1]) {
                                    if (stripos($nomeArquivo, 'shader') !== false) {
                                         ...
                                         if ($dataModify !== $dataInstalacao) {
                                            echo $bold .
$amarelo . "[!] Arquivo shader modificado: " . $nomeArquivo . "\n";
                                            echo $bold . $amarelo .
"[!] Horário da modificação: " . $dataModify . "\n"; 
                                            echo $bold . $amarelo .
"[*] Data de instalação do Free Fire: " . $dataInstalacaoFormatada . "\n";
                                            echo $bold . $branco .
"[#] Por favor, verifique no App Usage a data de instalação do Free Fire e compare com o horário da modificação. Se for diferente, aplique o W.O!\n\n";
                    ...
                    if ($encontrouBypass) {
                        echo $bold .
$amarelo . "[!] Modificação em arquivo de shaders detectada. Arquivo: " . $arquivoSuspeito . "\n";
                        echo $bold . $amarelo .
"[*] Hora da modificação: " . $dataModify . "\n";
                        echo $bold . $amarelo .
"[*] Verifique se a modificação ocorreu após a partida!\n\n";
                    }
                } elseif ($encontrouReplayPassado) {
                    echo $bold .
$vermelho . "[!] Possível modificação em arquivo de shaders detectada. Arquivo: " . $arquivoSuspeito . ", Horário: " .
$dataModify . "\n";
                    ...
                    echo $bold .
$fverde . "[i] Nenhuma alteração suspeita encontrada.\n";
                }
                ...
                if ($encontrouBypassPasta || $encontrouReplayPassadoPasta) {
                    echo $bold .
$vermelho . "[!] Modificação suspeita detectada na pasta shaders! Aplique o W.O!\n";
                    echo $bold . $amarelo .
"[*] Data da última modificação: " . $dataModifyFormatada . "\n\n";
                } else {
                    echo $bold .
$fverde . "[i] Pasta shaders sem alterações suspeitas.\n";
                    if (!empty($dataModifyFormatada)) {
                        echo $bold .
$amarelo . "[*] Data da última modificação: " . $dataModifyFormatada . "\n\n";
                    } else {
                        echo "\n";
                    }
                }
...
$bold . $amarelo . "[*] Data da última alteração na pasta 'gameassetbundles': " . ($dataChangeFormatada ?: "Não encontrada") . "\n";
                ...
                $comandoStat = 'adb shell stat ' .
escapeshellarg($diretorioVerificar) . ' 2>&1';
                $resultadoStat = shell_exec($comandoStat);
                ...
                if (strpos($resultadoStat, 'File:') !== false) {
                    ...
                    if ($matchAccess && $matchModify && $matchChange) {
                        ...
                        if ($dataAccess === $dataModify && $dataModify === $dataChange) {
                            echo $bold .
$vermelho . "[!] Possível bypass detectado - Datas idênticas\n";
                            echo $bold . $vermelho . "[i] Data: " . $dataModifyFormatada .
"\n";
                        } elseif ($dataModify === $dataChange) {
                            echo $bold .
$amarelo . "[i] Modificação da pasta: " . $dataModifyFormatada . "\n";
                        } else {
                            echo $bold .
$vermelho . "[!] Discrepância encontrada\n";
                            ...
                            if ($dateAccess && $dateChange) {
                                echo $bold .
$amarelo . "[i] Acesso: " . $dateAccess->format('d-m-Y H:i:s') . "\n";
                                echo $bold . $amarelo . "[i] Modificação: " .
$dataModifyFormatada . "\n";
                        ...
                        echo $bold .
$vermelho . "[!] Não foi possível extrair datas. Resultado:\n";
                        echo $bold . $amarelo . $resultadoStat . "\n";
                    }
                } elseif (strpos($resultadoStat, 'No such file') !== false) {
                    echo $bold .
$vermelho . "[!] Pasta não encontrada\n";
                } elseif (strpos($resultadoStat, 'Permission denied') !== false) {
                    echo $bold .
$vermelho . "[!] Sem permissão de acesso\n";
                } else {
                    echo $bold .
$vermelho . "[!] Erro desconhecido. Resultado:\n";
                    echo $resultadoStat . "\n";
                }

                echo $bold .
$branco . "[+] Caso a pasta 'android' esteja modificada após o fim da partida, aplique o W.O!\n\n";
                ...
                if ($resultadoVerificarPasta === "existe") {
                    ...
                    echo $vermelho .
"[*] Pasta 'gameassetbundles' não encontrada, verificando a pasta 'optionalavatarres'...\n";
                    $diretorioAlvo = $diretorioOptionalAvatarRes;
                    $nomePasta = "optionalavatarres";
                }

                $comandoDataModify = 'adb shell stat -c "%y" ' .
escapeshellarg($diretorioAlvo) . ' 2>/dev/null';
                ...
                if ($resultadoDataModify !== '') {
                    try {
                        ...
                        echo $bold . $amarelo . "[*] Data de modificação na pasta '$nomePasta': " .
$dataModificacao->format('d-m-Y H:i:s') . "\n";

                        $intervalo = $agora->getTimestamp() - $dataModificacao->getTimestamp();

                        if ($intervalo <= 3600) {
                            echo $bold .
$vermelho . "[!] Possível Bypass detectado! Modificada há menos de 1 hora.\n";
                            echo $vermelho .
"    Hora da modificação: " . $dataModificacao->format('H:i:s') . "\n";
                            echo $vermelho .
"    Hora atual:          " . $agora->format('H:i:s') . "\n";
                        }

                    } catch (Exception $e) {
                        echo $vermelho .
"[!] Erro ao extrair data de modificação da pasta '$nomePasta': " . $e->getMessage() . "\n";
                    }
                } else {
                    echo $vermelho .
"[!] Não foi possível obter a data de modificação da pasta '$nomePasta'.\n";
                }
                ...
                if ($resultadoArquivos !== '') {
                    ...
                    foreach ($arquivos as $arquivo) {
                        ...
                        $comandoDataModifyArquivo = 'adb shell stat -c "%y" ' .
escapeshellarg($caminhoArquivo) . ' 2>/dev/null';
                        ...
                        if ($resultadoDataModifyArquivo !== '' && $resultadoDataChangeArquivo !== '') {
                            try {
                                ...
                                if ($dataModifyArquivo != $dataChangeArquivo) {
                                    echo $bold .
$vermelho . "[!] Modificação detectada no arquivo: $nomeArquivo! Aplique o W.O!\n\n";
                                    $modificacaoDetectada = true;
                                }
                            } catch (Exception $e) {
                                echo $vermelho .
"[!] Erro ao verificar datas do arquivo $nomeArquivo: " . $e->getMessage() . "\n";
                            }
                        }
                    }
                    ...
                    if (!$modificacaoDetectada) {
                        echo $bold .
$fverde . "[i] Nenhuma alteração suspeita encontrada nos arquivos.\n\n";
                    }
                } else {
                    echo $vermelho .
"[*] Sem itens baixados! Verifique se a data é após o fim da partida!\n\n";
                }
                ...
                if (!empty($resultadoObb)) {
                    ...
                    foreach ($arquivosObb as $arquivo) {
                        ...
                        if (!empty($resultadoDataChange)) {
                            ...
                            echo $vermelho .
"[!] Não foi possível obter a data de modificação do arquivo OBB.\n";
                        }
                    }
                } else {
                    echo $vermelho .
"[*] OBB deletada e/ou inexistente!\n";
                }
                ...
                echo $bold .
$branco . "[+] Após verificar in-game se o usuário está de Wallhack, olhando skins de armas e atrás da parede, verifique os horários do Shaders e OBB e compare também com o horário do replay, caso esteja muito diferente as datas, aplique o W.O!\n\n";
...
$vermelho . "[!] Pareamento realizado de maneira incorreta, digite \"adb disconnect\" e refaça o processo.\n\n";
                ...
                if (!empty($comandoVerificarFF) && strpos($comandoVerificarFF, "com.dts.freefiremax") !== false) {
                    ...
                    echo $bold .
$vermelho . "[!] O FreeFire MAX está desinstalado, cancelando a telagem...\n\n";
                    exit;
                }
...
"[!] Bypass de renomear/substituir na pasta: $nomefinalpasta! Confira se o horário é após a partida, se sim, aplique o W.O!\n";
                ...
                $comandoFindBin = 'adb shell ls -t "/sdcard/Android/data/com.dts.freefiremax/files/MReplays" |
grep "\.bin$" | head -n 1';
                $arquivoBinMaisRecente = shell_exec($comandoFindBin);
