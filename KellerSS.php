      ...
      if ($opcaoscanner == "0")
        ...
        elseif ($opcaoscanner == "1") {
          ...
          echo $bold . $azul . "[+] Checando bypass de Wallhack/Holograma...\n";
            ...
            $arquivoBinMaisRecente = trim(shell_exec($comandoFindBin));
            ...
            if (!empty($arquivoBinMaisRecente)) {
                ...
                if ($matchAccessBin) {
                    ...
                    if ($bypassDetectado) {
                        echo $bold . $vermelho . "[!] Bypass holograma detectado, aplique o W.O!\n\n";
                    } else {
                    ...
                    if (strpos($resultadoStat, 'File:') !== false) {
                        ...
                        if ($matchAccess && $matchModify && $matchChange) {
                            ...
                            if ($accessDate === $modifyDate && $modifyDate === $changeDate) {
                                if ($firstInstallDate && $accessDate === $firstInstallDate) {
                                    continue;
                                }

                                echo $bold . $vermelho . "[!] Bypass Holograma detectado no arquivo: $nomeArquivo\n";
                                echo $bold . $vermelho . "[!] Aplique o W.O!\n\n";
                                continue;
                            }

                            if ($modify !== $change) {
                                $dateTimeChange = DateTime::createFromFormat('Y-m-d H:i:s', $change);
                                $dataChangeFormatadaLegivel = $dateTimeChange ? $dateTimeChange->format('d-m-Y H:i:s') : $change;
                    ...
                    if (!empty($resultadoStat) && strpos($resultadoStat, "No such file or directory") === false) {
                        ...
                        if (!empty($matchModify[1]) && !empty($matchChange[1]) && !empty($matchAccess[1])) {
                            ...
                            if ($diffInSeconds <= 3600) {
                                echo $bold . $vermelho . "[!] Possível bypass detectado: arquivo shader alterado recentemente.\n";
                                echo $bold . $vermelho . "[!] Arquivo: $nomeArquivo\n";
                                echo $bold . $vermelho . "[*] Hora da modificação: $dataModify\n";
                                echo $bold . $vermelho . "[*] Hora atual: " . $currentDateTime->format('d-m-Y H:i:s') . "\n\n";
